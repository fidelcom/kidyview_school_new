...
{
    function getStudentClassRank($student_id)
    {
            // $where = array('status'=>1);
        $student = getStudentDetails($student_id);
        $class_id = $student->childclass;

            $ci = &get_instance();
            $ci->load->database();
            $sql = "SELECT student_id,@rank := @rank + 1 rank FROM `result`, (SELECT @rank := 0) init WHERE class_id = $class_id ORDER BY `result`.`overall_percent`  DESC";
...
{
    function orderByTermsList($school_id)
    {
...
{
    function get_school_subscription_data($schoolID)
    {
		$ci = &get_instance();
		$ci->load->database();
		$ci->db->select("*");
