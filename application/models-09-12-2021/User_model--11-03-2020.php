<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class User_model extends CI_Model
	{
		...
		{
			$where = array('c.schoolId'=>$id,'c.status'=>1);
			$this->db->select('c.*, COUNT(ch.id) as num_child,CONCAT(t.teacherfname," ",t.teachermname," ",t.teacherlname) as teacher');
		...
		{
			$query = $this->db->query("SELECT s.* FROM ((SELECT DISTINCT(reciever) as user_id, `m`.`id` FROM `messages` `m` WHERE `school_id`='$id') UNION (SELECT DISTINCT(sender) as user_id,`m`.`id` FROM `messages` `m` WHERE `school_id`='$id'))s GROUP BY s.user_id ORDER BY id DESC");
			// lq();
			if($query->num_rows() > 0)
			{
				$data = $query->result();
