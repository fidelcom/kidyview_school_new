...
class Fees_model extends CI_Model {
    ...
    public function feesList($school_id)
    {
        $currentSession = get_current_session($school_id);
        $session_id = $currentSession->id;

        $where = array('session_id' => $session_id, 'school_id' => $school_id);
    ...
    public function paymentHistory($classID,$School_ID)
    {
        ...
        $where = array('class_id'=>$classID,'school_id'=>$School_ID);
        $this->db->where($where);
        $query = $this->db->get();
         if($query->num_rows() > 0)
            return $query->result();
