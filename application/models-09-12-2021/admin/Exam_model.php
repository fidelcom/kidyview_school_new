...
class Exam_model extends CI_Model {
    ...
    private function getStudentsMarksObtain($child_id,$exam_id)
    {
        ...
        // $this->db->join('school sc','sc.id = e.school_id','left');
        // $this->db->join('exam_marks_obtain em','e.id = em.exam_id','left');
        $this->db->where(['exam_id'=>$exam_id,'user_id'=>$child_id]);
        $query = $this->db->get();
        // lq();
        if($query->num_rows() > 0)
        {
    ...
    public function isExamExist($postData)
    {
        ...
        // prd($res);
        $where = array('e.subject_id'=>$postData['subject_id'],'e.class_id'=>$postData['class_id'],'e.status'=>'1','e.term_id'=>$postData['term_id'],'e.exam_mode' => $postData['exam_mode']);
        $this->db->select('e.*');
