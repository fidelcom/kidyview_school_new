<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Teacherdriver_model extends CI_Model
    {
        
        
     public function getdriversId($schoolID,$teacherID) 
     {
      $currentData = array();
      $query = $this->db->query("SELECT  GROUP_CONCAT(DISTINCT driverid ORDER BY driverid ASC) as driverid  FROM `driver_student_assignment` WHERE classid IN (SELECT assignclassteacher FROM teacher where id = '".$teacherID."' And schoolId = '".$schoolID."')");
      if ($query->num_rows() > 0) {   
       $currentData   = json_decode(json_encode( $query->row()), true);
      }
      return $currentData['driverid'];
     }
     
      public function driver($schoolID,$driverid)
      {
       $this->db->select(
       'd.id as driverID,         
       ...
       //echo $this->db->last_query();exit;
        ...
        public function studentlist($classid,$schoolID)
        { 
         $currentData = array();
         $query = $this->db->query("SELECT
                  
                  c.id as studentID,
                  dsa.driverid as driverid,
                  ...
                  inner join child as c on c.id = dsa.studentid 
                  where dsa.classid = '".$classid."' And dsa.schoolId = '".$schoolID."' group by dsa.studentid");
         if ($query->num_rows() > 0) {   
         $currentData   = json_decode(json_encode($query->result()), true);
         }
         return $currentData;
        }
      ...
      public function studentDriver($driverID,$schoolID){
   ...
   public function teacherDetail($schoolID,$teacherID)
   {
    $teachData = "";   
    $selectData = "SELECT * FROM `teacher` where id = '".$teacherID."' And schoolId = '".$schoolID."'";
    $query = $this->db->query($selectData);
    if ($query->num_rows() > 0) {
    $teachData  = json_decode(json_encode($query->row()), true);
    }
    return $teachData;
   }     
