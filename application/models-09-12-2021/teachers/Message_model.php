...
class Message_model extends CI_Model {
    public function getUser($userType='',$schoolId='',$userId=''){
        if($userType=='student'){
            $sql='SELECT assignclassteacher FROM teacher where id='.$userId.'';
            $query=$this->db->query($sql);
            $data_user = $query->row();
            $class_id=$data_user->assignclassteacher;
            $sql='SELECT ch.id,CONCAT(ch.childfname," " ,ch.childmname," " ,ch.childlname) as name from child ch where ch.childclass IN ('.$class_id.') AND status=1';
            $query=$this->db->query($sql);
            if($query->num_rows () > 0)
            {
            ...
            $sql="SELECT id,CONCAT(teacherfname,' ' ,teachermname,' ' ,teacherlname) as name FROM teacher where schoolId=$schoolId AND id !=$userId AND status=1";
            $query=$this->db->query($sql);
            if($query->num_rows () > 0)
            {
                $data_user = $query->result_array();
    ...
    function allData($user) {
        ...
        if($query->num_rows() > 0)
        {
           ...
           foreach ($tmpData as $key => $part) {
            $sort[$key] = strtotime($part['last_message']['created_date']);
            }
           array_multisort($sort, SORT_DESC, $tmpData); 
           ############ End Data By date ############### 
