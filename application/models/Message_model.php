...
class Message_model extends CI_Model {
    ...
    function users($user_id){
        ...
        if(isset($_GET['user_type']) && $_GET['user_type']=='Student'){
            ...
            $sql='SELECT ch.id,ch.childfname as fname,ch.childmname as mname,ch.childlname as lname from child ch where ch.childclass IN ('.$class_id.') AND class_session_id='.$session_id.' AND status=1';
            if(isset($_GET['name']))
    ...
    function addMessage($data) {
        $schoolId = $this->token->school_id;
        $data['session_id']= get_current_session($schoolId)->id;
...
//        }
        ...
        $query = $this->db->query("SELECT s.* FROM (
                                        ...
                                        AND m.sender_deleted = 0 AND session_id='$session_id' GROUP BY reciever)
                                        ...
                                        AND `m`.`receiver_deleted` = 0 AND session_id='$session_id' GROUP BY sender))s
