<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class User_model extends CI_Model
	{
		...
		{
			if($email!='' && $phone!='' && $memail!='' && $mphone!=''){
				$query = $this->db->query("SELECT id FROM parent WHERE ( fatheremail = '$email' OR fatherphone = '$phone' OR 'motheremail' = '$memail' OR 'motherphone' = '$mphone') ");
			}elseif($email!='' && $phone!='' && $memail==''){
				$query = $this->db->query("SELECT id FROM parent WHERE ( fatheremail = '$email' OR fatherphone = '$phone') ");
			}elseif($email=='' && $mphone!='' && $memail!=''){
				$query = $this->db->query("SELECT id FROM parent WHERE ( 'motheremail' = '$memail' OR 'motherphone' = '$mphone') ");
			}
			//echo $this->db->last_query();die;
			if($query->num_rows() > 0)
			{
		...
		public function getAllArticleForSchool($id)
		{
			...
			$this->emojidb->from("article t");
			$this->emojidb->where("t.schoolId",$schoolId);
		...
		public function getAllTimelineForSchool($id)
		{
			...
			$this->emojidb->where(["t.school_id"=>$schoolId,'is_deleted'=>'1']);
			//$this->db->where("t.status",1);
			$this->emojidb->order_by("t.id","DESC");
		...
		public function getcommentsDetail($id) {
			$data = array();
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->select('*');
			$this->emojidb->from('timeline_comment');
			$this->emojidb->where('timeline_id', $id);
		...
		public function getDiscussionAttachmentcommentsDetail($id) {
			...
			$data = array();
			$this->emojidb->select('*');
			$this->emojidb->from('discussion_comment');
        ...
        public function getSingleFeedbackDetails($id)
	{
            ...
            WHEN fb.user_type = 'parent' THEN parent.fatherlname 
            END as lname,
            CASE  WHEN fb.user_type = 'driver' THEN driver.driveremail
            END as driveremail,
            CASE  WHEN fb.user_type = 'driver' THEN driver.driverphone 
            END as driverphone
            ...
            LEFT OUTER JOIN school as school ON school.id = fb.school_id 
            WHERE fb.id = '".$id."'"; 
            $query = $this->emojidb->query($dataQuery);
            if($query->num_rows() > 0)
            {
            return $query->row();
            }
		...
		public function getAllSessionForSchool($id)
		{
			...
			if($query->result_id->num_rows!=0)
			{
				$data_user = $query->result_array();
				for($i=0;$i<count($data_user);$i++){
					$data_user[$i]['is_previous']=0;
					if($data_user[$i]['sessionend']<date('Y-m-d')){
						$data_user[$i]['is_previous']=1;	
					}
				}
				return $data_user;
			}
		...
		public function getAllClassForSchool($id,$allData="")
		{
			if($allData=='All'){
				$where = array('c.schoolId'=>$id,'s.current_sesion'=>1);
			}else{
		...
		{
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->select("d.*, dt.name as discussion_type_text,dl.total_like");
			...
			$this->emojidb->where("d.school_id",$id);
		...
		{
			$currentSession = get_current_session($id);
        	$session_id = isset($currentSession) && $currentSession->id?$currentSession->id:'';
			$query = $this->db->query("SELECT s.* FROM ((SELECT DISTINCT(reciever) as user_id, `m`.`id` FROM `messages` `m` WHERE `school_id`='$id') UNION (SELECT DISTINCT(sender) as user_id,`m`.`id` FROM `messages` `m` WHERE `school_id`='$id' AND session_id='$session_id'))s GROUP BY s.user_id ORDER BY s.id desc");
			// lq();
			if($query->num_rows() > 0)
			{
				$data = $query->result();
		...
		function lastMessage($user){
			...
			$this->emojidb->where("m.reciever", $user);
			$this->emojidb->group_by("m.id","DESC");
		...
		public function getAllSubjectForSchool($id,$class_id='')
		{
			...
			if($class_id!=''){
				$this->db->where('s.class', $class_id);
			}
		...
		{
			$this->db->select('school.*, cc.name as countryName,cc.phonecode as countryCode');
			$this->db->where('school.id',$id);
			$this->db->join('country_codes cc', 'cc.id = school.country', 'LEFT');
                        $this->db->from('school as school');
			$query = $this->db->get();
			if($query->num_rows() > 0)
			{
				$school = $query->row();
                                $school->currencies =  $this->getCountryCurrency($school->country);
...
//			);
       ...
       return $newData; 
		...
		{
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->select('d.*, ss.school_name, dc.name as category_name');
			$this->emojidb->from('discussion d');
			$this->emojidb->where('d.id', $id);
			$this->emojidb->join('school ss', 'd.school_id = ss.id', 'LEFT');
			$this->emojidb->join('discussion_category dc', 'd.discussion_type = dc.id', 'LEFT');
			$query = $this->emojidb->get();
			if($query->num_rows() > 0)
			{
				$data = $query->row();
		...
		public function parentDisabled($data,$id)
		{
			...
			else
			{
                ...
                public function changeParentStatus($data,$id)
		{
			$this->db->where('id',$id);
			$this->db->update('parent',$data);
			//echo $this->db->last_query(); die;
		...
		{
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->where('id',$timelineID);
			$this->emojidb->where(['school_id'=>$schoolId,'is_deleted'=>'1']);
			$this->emojidb->update('timeline',$data);
			//echo $this->db->last_query(); die;
			return ($this->emojidb->affected_rows()) ? 1: 0;
		...
		{
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->insert('article',$data);
			$id = $this->emojidb->insert_id();
			return $id;
		}
		...
		{
                        $this->emojidb = $this->load->database('emojidb', TRUE);
			$this->emojidb->insert('album',$data);
			$id = $this->emojidb->insert_id();
			return $id;
		}
		...
		{
			$this->db->where('id',$userID);
			return $this->db->update('school',array('password'=> md5($password)));
		}
		
		public function resetPasswordSchoolAdmin($userID,$password)
		{
			$this->db->where('id',$userID);
			return $this->db->update('school_admin',array('password'=> md5($password)));
		}
		...
		public function studentBirthday_calendar_data($school_id='',$calendardate,$iscalendardata){  
			...
			$this->db->select("concat(c.childfname,c.childmname,c.childlname) as title,c.childdob as start");
			$this->db->from('child c');
			$this->db->join('sessions si','c.class_session_id = si.id','left');
			$this->db->where(array('si.current_sesion'=>1));
			$this->db->where('c.schoolId',$school_id);
			if($iscalendardata==0){
				
				$this->db->where("c.childdob like '%$calendardate'");
		...
		function getallconversation($sender, $reciever){
			...
			$this->emojidb->select("m.*");
			$this->emojidb->from("messages m");
			$this->emojidb->where("((m.sender = '$sender' AND  m.reciever = '$reciever') OR (m.sender = '$reciever' AND  m.reciever = '$sender'))");
			$this->emojidb->group_by("m.id","DESC");
			$this->emojidb->order_by("m.created_date","DESC");
			$query = $this->emojidb->get();
			//echo $this->db->last_query();
			$this->load->helper('common');
			$data = $query->result();
		...
		public function getAllUserForMessage($schoolId, $userType,$user_for=array())
		{
			$this->db->where('school_id', $schoolId);
			if(!empty($user_for)){
				$this->db->where_in('user_type', $user_for);
			}else{
       ...
       public function getTermReport($reportInput) 
      {
       ...
       {
            $this->db->select("*");
            $this->db->where('phonecode!=',NULL);
            $this->db->where('phonecode!=',0);
        ...
        public function addsponser($data) {
        ...
        public function deleteSponser($id)
	{
        ...
        $this->db->insert('admin_currency', $data);
        $id = $this->db->insert_id();
        return $id;
        }
	...
	{
            $this->db->where('id', $id);
            $query = $this->db->delete('admin_currency');
            return $query;
        }
	...
	{
            $this->db->where('id',$id);
            $this->db->update('admin_currency',$data);
            //echo $this->db->last_query(); die();
            if($this->db->affected_rows())
            {
                    return true;
        ...
        public function mapcurrency($data) {
        $this->db->insert('map_currency', $data);
        $id = $this->db->insert_id();
        return $id;
        }
        
        public function unmapCurrencyList($countryID) 
        {   
            $dataQuery = "select * from admin_currency where id Not IN (select currency_id from map_currency where country_id=".$countryID.")";
            $query = $this->db->query($dataQuery); 
           if($query->num_rows() > 0)
              return $query->result_array();
             else
        ...
        public function getCurrencyList($countryID) 
        {
            $dataQuery = "select ac.*,cc.name as countryName from admin_currency as ac
                    left join country_codes as cc on cc.id = '".$countryID."'  
                    where ac.id IN (select currency_id from map_currency where country_id=".$countryID.")";
            $query = $this->db->query($dataQuery); 
           if($query->num_rows() > 0)
              return $query->result_array();
             else
        ...
        public function unMapCurrency($id)
	{
            $this->db->where('currency_id', $id);
            $query = $this->db->delete('map_currency');
            return $query;
        }

        
        public function getTransactionList($schoolid)
        {
            if(empty($schoolid) || $schoolid==0)
            return false;    
            $userId = 'S-'.$schoolid;
            $this->db->select('pay.*, sub.name as sub_name,sub.type as sub_type,sub.validity as sub_validity,sub.no_of_student as sub_no_of_student,sub.amount as sub_plan_amount,sub.currency as sub_currency,school.school_name as school_name');
            $this->db->from('payment as  pay');
            $this->db->join('subscription as sub', 'sub.id = pay.plan_id', 'LEFT');
            $this->db->join('school as school', 'school.id = '.$schoolid.'', 'LEFT');
             $this->db->where('pay.user_id',$userId);
            ...
            $transaction[$i]['decoded_currency_log'] = unserialize($value['currency_log']);
            $transaction[$i]['payment_date']   = date('m-d-Y h:i A', strtotime($value['payment_date']));
            $transaction[$i]['last_date']   = date('m-d-Y h:i A', strtotime($value['last_date']));
            $i++;
            }
            return $transaction;
            }
        ...
        public function changeTeacherStatus($data,$id)
	{
            $this->db->where('id',$id);
            $this->db->update('teacher',$data);
            //echo $this->db->last_query(); die;
