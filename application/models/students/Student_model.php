...
class Student_model extends CI_Model {
    ...
    public function getStudentSubmitAssignmentList($classID='',$user_id=''){
        $sql='SELECT ast.*,CONCAT(c.class," " ,c.section) as classname,s.subject ,CONCAT(t.teacherfname," " ,t.teachermname," " ,t.teacherlname) as teachername,asu.id as asid,asu.submission_date as datesubmited FROM class c inner join subjects s on c.id=s.class inner join teacher t on s.teacher=t.id inner join assignment ast on ast.subject_id=s.id inner join assignment_submission asu on asu.assignment_id=ast.id inner join sessions si on ast.session_id=si.id  where si.current_sesion= 1 AND asu.user_id='.$user_id.' AND ast.class_id='.$classID.'';
        $query=$this->db->query($sql);
        if($query->num_rows() > 0)
        {
            $data_user = $query->result_array();
    ...
    public function checkExistAssignment($assignment_id='',$user_id='')
        ...
        public function getClassboardList($action='',$student_id='',$offset,$perpage,$classID=''){
            $sql="SELECT cb.*,CONCAT(c.class,' ' ,c.section) as classname from classboard cb inner join classboard_student cs on cb.id=cs.classboard_id inner join class c on cb.class_id=c.id left join sessions si on cb.session_id=si.id where si.current_sesion= 1 AND cb.class_id='".$classID."' AND cs.student_id='".$student_id."' order by cb.id DESC";
            if($action=='data'){
        ...
        public function getAssignedTeacher($subject_id=''){
            $this->db->select('t.id,CONCAT(t.teacherfname," " ,t.teachermname," " ,t.teacherlname) as teachername,teacheremail');
