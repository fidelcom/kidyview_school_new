...
class Message_model extends CI_Model {
    ...
    function users($user_id){
        ...
        $class_id=$this->token->class_id;
        $student_id=$this->token->student_id;
       if(isset($_GET['user_type']) && $_GET['user_type']=='Teacher'){
            $sql="SELECT id,teacherfname as fname,teachermname as mname,teacherlname as lname FROM teacher where schoolId=".$this->token->school_id." AND (find_in_set($class_id,assignclassteacher) AND 1) AND status=1";
            if(isset($_GET['name']))
        ...
        elseif(isset($_GET['user_type']) && $_GET['user_type']=='Parent'){
            ...
            $sql='SELECT ch.parent_id from child ch where ch.childclass = '.$class_id.' AND id != '.$student_id.' AND parent_id !='.$user_id.' AND status=1';
