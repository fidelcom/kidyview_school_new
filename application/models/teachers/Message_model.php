...
class Message_model extends CI_Model {
    public function getUser($userType='',$schoolId='',$userId=''){
        ...
        if($userType=='student'){
            ...
            $sql='SELECT ch.id,CONCAT(ch.childfname," " ,ch.childmname," " ,ch.childlname) as name from child ch where ch.childclass IN ('.$class_id.') AND class_session_id='.$session_id.' AND status=1';
            $query=$this->db->query($sql);
            if($query->num_rows () > 0)
            {
