<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Teacher_model extends CI_Model
	{
		...
		public function getStudentList($teacher_id='',$school_id=''){
		...
		public function getStudentByClass($teacher_id='',$class_id=''){
            ...
            $sql='SELECT ch.id,CONCAT(ch.childfname," " ,ch.childmname," " ,ch.childlname) as childname FROM child ch inner join (select id,class,section from class where id in ('.$classid.')) c on ch.childclass=c.id where AND ch.class_session_id='.$session_id.' group by ch.id';
			$query=$this->db->query($sql);
            if($query->num_rows() > 0)
            {
                $data_user = $query->result_array();
		...
		public function getClassboardList($action='',$teacher_id='',$offset,$perpage,$school_id=''){
			$currentSession = get_current_session($school_id);
        	$session_id = isset($currentSession) && $currentSession->id?$currentSession->id:'';
		
			$sql="SELECT cb.*,CONCAT(c.class,' ' ,c.section) as classname,s.school_name  from classboard cb inner join class c on cb.class_id=c.id inner join school s on c.schoolId=s.id where cb.teacher_id='".$teacher_id."' AND cb.session_id= '".$session_id."' order by cb.id DESC";
			//$sql .= ' AND cb.session_id="'.$session_id.'"';
			if($action=='data'){
		...
		public function getAssignmentList($teacher_id='',$filterbydate='',$class_id='',$subject_id='',$category='',$school_id=''){
			$currentSession = get_current_session($school_id);
        	$session_id = isset($currentSession) && $currentSession->id?$currentSession->id:'';
			$sql='SELECT ast.*,CONCAT(c.class," " ,c.section) as classname,s.subject  FROM assignment ast left join class c on ast.class_id=c.id left join subjects s on ast.subject_id=s.id where ast.status=1';	
			if($class_id!='') {
		...
		public function getTeacherSubject($class_id='',$teacher_id=''){
			
            $sql='select s.subject,s.id from subjects s where s.class="'.$class_id.'" AND s.teacher="'.$teacher_id.'"';
			$query=$this->db->query($sql);
            if($query->num_rows() > 0)
            {
                $data_user = $query->result_array();
		...
		public function getTeacherSubjectClass($teacher_id=''){
            ...
            $sql='SELECT c.id,CONCAT(c.class," " ,c.section) as classname from class c inner join subjects s on c.id=s.class where s.teacher="'.$teacher_id.'" group by c.id';
			//echo $sql;die;
			$query=$this->db->query($sql);
            if($query->num_rows() > 0)
            {
		...
		public function getStudentSubmitedAssignmentList($schoolID='',$teacher_id='',$class_id='',$subject_id='',$category=''){
		...
		{
			$schoolId=$this->token->school_id?$this->token->school_id:$this->session->userdata('user_data')['id'];
			$session_id= get_current_session($schoolId)->id;
