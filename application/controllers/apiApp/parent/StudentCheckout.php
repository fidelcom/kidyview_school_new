                            ...
                            $this->firebases->sendNotification($token, $title, $message);
						        ...
						        // prd($notify);  die;
										   ...
										   //$response = $client->messages->create('+19709844629', array($guardianPhone), $otp . " is your otp checkout code for $childName ($childRegisterId). OTP is confidential.", array("url" => "http://foo.com/sms_status/"));
											$msg= $otp . " is your otp checkout code for $childName ($childRegisterId). OTP is confidential.";
										    $response = sendKidyviewSms($guardianPhone,$msg);
						        // echo $response; die;
						       ...
						       // {
						       		 if(!empty($guardianEmail))
					                {
										...
										if(sendKidyviewEmail($guardianEmail,'','','','Kidyview - Checkout OTP', $message)){
					                        $mailsent = true;
					                    }
					                }
					                $return['success'] = "true";
				        ...
				        if($result){
				        	...
				        	// Setup Push notification
				        		...
				        		if(!empty($parentmotherphone))
				        		{
									...
									$msg= $otp . " is your otp checkout code for $childName ($childRegisterId). OTP is confidential.";
									$response = sendKidyviewSms($parentmotherphone,$msg);
								}
				                ...
				                if(!empty($parentfatheremail))
				                {
									...
									if(sendKidyviewEmail($parentfatheremail,'','','','Kidyview - Checkout OTP', $message)){
				                        $mailsent = true;
				                    }
				                } 
				                if(!empty($parentmotheremail))
				                {
									...
									if(sendKidyviewEmail($parentmotheremail,'','','','Kidyview - Checkout OTP', $message)){
				                        $mailsent = true;
				                    }
				                }
				                $return['success'] = "true";
