...
class StudentAttendance extends REST_Controller {
		...
		// print_r($postData);die;
         ...
         $result  = $this->model->checkinStudentAttendance($studentAttendance);
           if ($result || (count($alreadyCheckIn)>0)) {
					...
					foreach($studentAttendance as $sdata){
						$child_id=$sdata['student_id'];
						$parent_data=getChildParent($child_id);
						$pnotificationData['type']="checked in";
						$pnotificationData['pname']=isset($parent_data['fatherfname']) && $parent_data['fatherfname']!='' ? $parent_data['fatherfname'] : $parent_data['motherfname'];
						$pnotificationData['pemail']=isset($parent_data['fatheremail']) && $parent_data['fatheremail']!='' ? $parent_data['fatheremail'] : $parent_data['motheremail'];
						$message = $this->load->view('emailtemplate/childCheckinCheckoutTemplate',$pnotificationData, true);
                        sendKidyviewEmail($pnotificationData['pemail'],'','','', 'Checked in Your Child', $message);
					}
					}
					$return['success'] = "true";
	...
	public function manualCheckout_put()
	{
         ...
         foreach ($postData as $checkout) {
            ...
            if ($result) {
					/*foreach($postData as $sdata){
						$child_id=$sdata['student_id'];
						$parent_data=getChildParent($child_id);
						$pnotificationData['type']="Checked out";
						$pnotificationData['pname']=isset($parent_data['fatherfname']) && $parent_data['fatherfname']!='' ? $parent_data['fatherfname'] : $parent_data['motherfname'];
						$pnotificationData['pemail']=isset($parent_data['fatheremail']) && $parent_data['fatheremail']!='' ? $parent_data['fatheremail'] : $parent_data['motheremail'];
						$message = $this->load->view('emailtemplate/childCheckinCheckoutTemplate',$pnotificationData, true);
						//sendKidyviewEmail($pnotificationData['pemail'],'','','', 'Checked out Your Child', $message);
					}*/
					$return['success'] = "true";
