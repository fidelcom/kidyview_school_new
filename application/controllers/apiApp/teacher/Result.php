...
class Result extends REST_Controller {
    ...
    public function studentActivityTermsResult_post()
    {
        $postData = json_decode(file_get_contents('php://input'), true);
         if ($postData == '') {
                $postData = $_POST;
        ...
        $studentResult = $this->model->getStudentsActivityTermsResult($postData);
       
        if (count($studentResult) > 0) {
            $return['success'] = "true";
            $return['message'] = "Student Activities Result.";
            $return['GrandResult'] = $studentResult['grandResult'];
            $return['data'] = $studentResult['result'];
            $return['error'] = $this->error;
