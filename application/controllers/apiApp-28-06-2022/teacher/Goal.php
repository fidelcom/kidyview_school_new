...
class Goal extends REST_Controller {
    ...
    public function index_get(){
        $data = $this->goal_model->allData();
        $total = $this->goal_model->allDataCount();
        $return['success'] = "true";
        $return['title'] = "success";
    ...
    public function index_post() {
        ...
        $formData['points'] = $postData['points'];
    ...
    public function update_post() {
        ...
        $this->data = $id;
        $this->goal_model->update($id,$formData);
        
        if ($id) {
            if (!empty($arrPhoto)) {
                $this->db->where("goal_id",$id);
                $this->db->delete('goals_attachment');
                foreach ($arrPhoto as $photoData)
                {
                    $photoData['goal_id'] = $id;
                    $photoId = $this->goal_model->addAttachment($photoData);
                }
            }
    ...
    public function status_post() {
        ...
        $this->data = $id;
        $detail = $this->goal_model->detail($id); 
        // lq();
        $this->goal_model->update($id,$formData);
         // prd($detail);
        ...
        {
            if($formData['status'] == 'Achived')
            {
                $user_id = "T-".$this->token->user_id;
                //$this->goal_model->addPoints($user_id,$detail);
            }
            ...
            $return['error'] = (object) $this->error;
            $return['data'] = $this->goal_model->detail($id);
            $this->response($return, REST_Controller::HTTP_OK);
        }
        else
        {        
            $return['success'] = "false";
            $return['title'] = "error";
            $return['message'] = "Goal status not updated.";
            $return['error'] = (object) $this->error;
            $return['data'] = (object) $this->data;
        }
    ...
    public function points_post() {
        ...
        $this->form_validation->set_rules('student_id', 'Student', 'trim|required');
        $this->form_validation->set_rules('points', 'Points', 'trim|required');
        $this->form_validation->set_rules('transection_type', 'Type', 'trim|required');
        ...
        $formData['points'] = $postData['points'];
        $formData['school_id'] = $this->token->school_id;
        $user_id = $user_id = "T-".$this->token->user_id;
        $formData['created_date'] = date("Y-m-d H:i:s");
        
        $res = $this->goal_model->addPoints($user_id,$formData);
        ...
        {        
            $return['success'] = "false";
            $return['title'] = "error";
            $return['message'] = "Points not updated.";
            $return['error'] = (object) $this->error;
            $return['data'] = (object) $this->data;
        }
    ...
    public function student_award_list_get(){
        $data = $this->goal_model->student_award_list();
        $total = $this->goal_model->allAwardCount();
        $return['success'] = "true";
        $return['title'] = "success";
        $return['message'] = "Record found successfully.";
        $return['data'] = $data;
    ...
    public function share_points_post() {
        $postData = json_decode(file_get_contents('php://input'), true);
        if ($postData == '') {
            $postData = $_POST;
        }
        ...
        $this->form_validation->set_data($postData);
        $this->form_validation->set_rules('student_id', 'Student', 'trim|required');
        $this->form_validation->set_rules('id', 'Goal', 'trim|required');
        $this->form_validation->set_rules('points', 'Points', 'trim|required');
        //$this->form_validation->set_rules('transection_type', 'Type', 'trim|required');
        ...
        $formData['transection_type'] = "Goal Achived";
        $this->db->insert('student_point',$formData);
        ...
        {                        
            $return['success'] = "true";
            $return['title'] = "success";
            $return['message'] = "Points shared successfully.";
            $return['error'] = (object) $this->error;
            $return['data'] = array();
            $this->response($return, REST_Controller::HTTP_OK);
        }
