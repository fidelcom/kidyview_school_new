...
class Cron extends CI_Controller {
            ...
            for($i=0;$i<count($assignmentData);$i++){
                ...
                if (!empty($getClassStudentData)) {
                    foreach ($getClassStudentData as $studentData)
                    {
                        ...
                        $this->sendMail($studentEmail, "Your Child Assignment", $message);
                        }
                    }
                    }
                }  
            ...
            for($i=0;$i<count($assignmentData);$i++){
                ...
                if (!empty($getClassStudentData)) {
                    foreach ($getClassStudentData as $studentData)
                    {
                        ...
                        if($parentEmail!='' && (!empty($isParentNotify) && $isParentNotify->is_web==1)){
                        $message = $this->load->view('emailtemplate/commonTemplate',$pnotificationData, true);
                        $this->sendMail($parentEmail, "Your Child Project", $message);
                        }
                    }
                    }
                }  
            ...
            for($i=0;$i<count($assignmentData);$i++){
                ...
                if (!empty($getClassStudentData)) {
                    foreach ($getClassStudentData as $studentData)
                    {
                        ...
                        if($parentEmail!='' && (!empty($isParentNotify) && $isParentNotify->is_web==1)){
                        $message = $this->load->view('emailtemplate/commonTemplate',$pnotificationData, true);
                        $this->sendMail($studentEmail, "Your Child Project", $message);
                        }
                    }
                    }
                }  
    ...
    public function getTwentyFourHoursExamNotification() {
        ...
        $this->db->select('*');
        $this->db->from('exam');
        $this->db->where('last_submission_date',$tomorrowDate);
        $this->db->where('status','1');
        $query=$this->db->get();
        if($query->num_rows()>0)
        {
    ...
    public function getBeforeTwoHoursExamNotifications() {
        ...
        $this->db->select('*');
        $this->db->from('exam');
        $this->db->where('last_submission_date',$date);
        $this->db->where('status','1');
        $query=$this->db->get();
        if($query->num_rows()>0)
        {
