...
class Auth extends REST_Controller {
    ...
    public function signupSchool_post() {
        ...
        if (isset($postData['email']) && $postData['school_name'] && $postData['phone']) {
            ...
            if ($this->user->schoolExist($postData['email'])) {
                ...
                $mailResponse = sendKidyviewEmail($postData['email'],'','','','KidyView School Registration', $message);
    ...
    public function forget_password_post() {
        ...
        if (isset($postData['email'])) {
            ...
            if ($query->num_rows() == 1) {
                ...
                $mailResponse = sendKidyviewEmail($email,'','','','KidyView : Forget Password', $message);
                if ($mailResponse) {
    ...
    $usertype=$postData['usertype'];
        ...
        if ($userData != '') {
			$getsubscription=get_subscription($userData['school_id']);
			/*if($getsubscription==0){
				$return['status'] = 400;
				$return['success'] = "false";
				$return['data'] = $userData['school_id'];
				$return['message'] = "You have not a subscription plan.Please select a subscription plan.";
				$return['error'] = 'notSubscribed';
				$this->response($return, REST_Controller::HTTP_BAD_REQUEST);
			}*/
            $this->error = $this->form_validation->error_array();
            if(isset($userData['is_email_verified']) && $userData['is_email_verified']==0 && $usertype=="school"){
            $return['status'] = 400;
            $return['success'] = "false";
            $return['message'] = "Your email is not yet verified, kindly contact hello@kidyview.com to verify your email.";
            $return['error'] = $this->error;
            $this->response($return, REST_Controller::HTTP_BAD_REQUEST);
            }
			$this->load->library('user_agent');
			...
			$expieryDateTime = $datetime->format('Y-m-d H:i:s');
            if($usertype=="school"){   
			$userTokenStr = $userData['id']. "#" . $expieryDateTime . "#" . $platform. "#". $userData['school_id']. "#". $usertype. "#". $userData['subscription_id'];
            }else{
                $userTokenStr = $userData['id']. "#" . $expieryDateTime . "#" . $platform. "#". $userData['school_id']. "#". $usertype. "#sudadmin";   
            }
            $userToken = $this->settings->encryptString($userTokenStr);

			$dataUserToken = array(
				"user_id" => $userData['school_id'],
				"token" => $userToken,
				"device" => $platform,
				"fcm_key" => isset($postData['fcm_key']) && $postData['fcm_key']!=''?$postData['fcm_key']:''
			);
			$this->user->addToken($dataUserToken);
            $data['is_logged_in'] = true;
            $data['user_data'] = $userData;
	...
	public function checkSchoolEmail_post() {
        //require 'phpmailer/Send_Mail.php';

        $postData = json_decode(file_get_contents('php://input'), true);
        $res = new blankClass();
        if (isset($postData['email']) && $postData['school_name'] && $postData['phone'] && $postData['password']) {
            $postData = $this->security->xss_clean($postData);
            $this->form_validation->set_data($postData);
