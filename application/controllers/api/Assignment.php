...
// prd($result);
	...
	public function createAssignment_post() {
		...
		$currentSession = get_current_session($sid);
		...
		if ($add) {
			...
			if (!empty($getClassStudentData)) {
				foreach ($getClassStudentData as $studentData)
				{
					...
					$pnotificationData['sender_id'] = "S-".$sid;
					$pnotificationData['to_do_id'] = $add;
					//$pnotificationData['school_id'] = $postData['school_id'];
					$pnotificationData['message'] = "Assignment created ".$postData['title']." for your child.";
					...
					$isParentNotify=notificationSettingHelper($postData['school_id'],$student_id,'Assignment');
