<?php
	...
	class Message extends REST_Controller {
		...
		public function sendMessage_post() {
			...
			$sid = $sdata['id'];
			$currentSession = get_current_session($postData['schoolID']);
			...
			//print_r($postData['receiver_id']);die;
				if (!empty($postData['receiver_id'])){
					...
					if($add) {
						...
						$schooluser=explode('-',$user);
						$schooluser=$schooluser[0];
						if($schooluser=='S'){
							$notificationData['school_id'] = $postData['schoolID'];
						}
		...
		public function getMessageList_post() {
			...
			$result = $this->model->allData($userId,$school_id);
				if ($result) {
						$return['success'] = "true";
