<?php
	...
	public function getProjectList_post() {
		...
		$result = $this->Project_model->getProjectList($id,$filterbydate='',$class_id,$subject_id,$category,$school_id);
	...
	public function getProjectDetails_post() {
		...
		$id=$postData['id'];
		$result = $this->Project_model->getProjectDetails($id);
		...
		if ($add) {
			...
			if (!empty($getClassStudentData)) {
				foreach ($getClassStudentData as $studentData)
				{
					...
					$notificationData['message'] = $teacherData->teacherfname." created ".$postData['title']." project for you.";
					...
					$pnotificationData['message'] = $teacherData->teacherfname." created ".$postData['title']." project for your child.";
			...
			$isNotify=notificationSettingHelper($postData['school_id'],$user_id,'Project');
			$schoolNotificationData['receiver_id'] = $user_id;
			$schoolNotificationData['sender_id'] = "T-".$tid;
			$schoolNotificationData['to_do_id'] = $add;
			$schoolNotificationData['school_id'] = $postData['school_id'];
			$schoolNotificationData['message'] = $teacherData->teacherfname." created ".$postData['title']." project.";
			$schoolNotificationData['type'] = "project";
			$schoolNotificationData['url'] = "project-detail/".$encryptedUrl;
			if(!empty($isNotify) && $isNotify->is_push==1){
			...
			//$this->sendMail($schoolEmail, "New Project", $message);
			sendKidyviewEmail($schoolEmail,'','','','New Project', $message);
			}
	...
	public function getStudentSubmitedProjectList_post() {
		...
		$subject_id=$postData['subject_id'];
		$category=$postData['category'];
		$result = $this->Project_model->getStudentSubmitedProjectList($schoolID,$id,$class_id,$subject_id,$category);
	...
	public function getSubmitedProjectDetails_post() {
		...
		$id=$postData['id'];
		$result = $this->Project_model->getSubmitedProjectDetails($id);
	...
	public function assignMarks_post() {
		...
		if ($update) {
			...
			foreach ($getClassStudentData as $studentData)
			{
				...
				//$this->sendMail($parentEmail, "Your Child New Graded Assignment", $message);
			...
			$schoolNotificationData['school_id'] = $postData['school_id'];
			$schoolNotificationData['message'] = $postData['title']." project has been graded.";
			$schoolNotificationData['type'] = "project";
