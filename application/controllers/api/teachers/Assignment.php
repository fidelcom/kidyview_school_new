<?php
	...
	public function getAssignmentList_post() {
		...
		$result = $this->Teacher_model->getAssignmentList($id,$filterbydate='',$class_id,$subject_id,$category,$school_id);
	...
	public function getAssignmentDetails_post() {
		...
		$id=$postData['id'];
		$result = $this->Teacher_model->getAssignmentDetails($id);
	...
	public function getTeacherSubjectClass_post() {
		...
		$data = $this->session->userdata('teacher_data');
		$id = $data['id'];
		$result = $this->Teacher_model->getTeacherSubjectClass($id);
			if ($result) {
					$return['success'] = "true";
	...
	public function getTeacherSubject_post() {
		...
		$result = $this->Teacher_model->getTeacherSubject($class_id,$id);
			if ($result) {
					$return['success'] = "true";
					$return['message'] = "Class found.";
					$return['data'] = $result;
	...
	public function createAssignment_post() {
		...
		$tid = $tdata['id'];
		$teacherData = $this->Teacher_model->getTeacherDetails($tid);
		$currentSession = get_current_session($postData['school_id']);
		...
		if ($add) {
			...
			if (!empty($getClassStudentData)) {
				foreach ($getClassStudentData as $studentData)
				{
					...
					$notificationData['sender_id'] = "T-".$tid;
					$notificationData['to_do_id'] = $add;
					$notificationData['school_id'] = $postData['school_id'];
			...
			$user_id="S-".$postData['school_id'];
			$isNotify=notificationSettingHelper($postData['school_id'],$user_id,'Assignment');
			...
			$schoolEmail='';
			$schoolData=getSchoolDetails($postData['school_id']);
			if(!empty($schoolData)){
			...
			$message = $this->load->view('emailtemplate/commonTemplate',$notificationData, true);
			//$this->sendMail($schoolEmail, "New Assignment", $message);
	...
	public function getStudentSubmitedAssignmentList_post() {
		...
		$subject_id=$postData['subject_id'];
		$category=$postData['category'];
		$result = $this->Teacher_model->getStudentSubmitedAssignmentList($schoolID,$id,$class_id,$subject_id,$category);
	...
	public function getSubmitedAssignmentDetails_post() {
		...
		$id=$postData['id'];
		$result = $this->Teacher_model->getSubmitedAssignmentDetails($id);
		...
		if ($update) {
			...
			if (!empty($getClassStudentData)) {
				foreach ($getClassStudentData as $studentData)
				{
					...
					$notificationData['message'] = $teacherData->teacherfname." have changed due date for ".$postData['title']." assignment.";
					$notificationData['type'] = "assignment";
					$notificationData['url'] = "assignment-detail/".$encryptedUrl;
					if(!empty($isNotify) && $isNotify->is_push==1){
					$this->Teacher_model->add($notificationData,'notifications');
	...
	public function assignMarks_post() {
		...
		$termData =get_session_n_terms($postData['school_id'],$postData['submission_date']);
		 if($termData>0){
		$postData['term_id']=isset($termData) && $termData->term_id!=''?$termData->term_id:'';
		...
		if ($update) {
			...
			$encryptedUrl=$this->encryptData($text,$pass);
			$getClassStudentData=$this->Teacher_model->getStudentDetails($postData['user_id']);
			$studentEmailArray=array();
			if (!empty($getClassStudentData)) {
			foreach ($getClassStudentData as $studentData)
			{
				...
				$notificationData['message'] = " Your ".$postData['title']." assignment has been graded.";
				$notificationData['type'] = "assignment";
				$notificationData['url'] = "submit-assignment-detail/".$encryptedUrl;
				if(!empty($isNotify) && $isNotify->is_push==1){
				$this->Teacher_model->add($notificationData,'notifications');
				}
				/* Email to child */
				...
				if($studentEmail!='' && (!empty($isNotify) && $isNotify->is_web==1) ){
				$message = $this->load->view('emailtemplate/commonTemplate',$notificationData, true);
				//$this->sendMail($studentEmail, "Your graded assignment", $message);
				sendKidyviewEmail($studentEmail,'','','','Your graded assignment', $message);
				}
				$getParentData=$this->Teacher_model->getParentData($studentData['id']);
				if($getParentData){
				$pnotificationData['receiver_id'] = "P-".$getParentData->id;
				...
				$pnotificationData['message'] = $postData['title']." assignment has been graded for your child.";
				$pnotificationData['type'] = "assignment";
				$pnotificationData['url'] = "submit-assignment-detail/".$encryptedUrl;
				$isParentNotify=notificationSettingHelper($postData['school_id'],$pnotificationData['receiver_id'],'Assignment');
				if(!empty($isParentNotify) && $isParentNotify->is_push==1){
				$this->Teacher_model->add($pnotificationData,'notifications');
				}
			...
			$schoolNotificationData['sender_id'] = "T-".$tid;
			$schoolNotificationData['to_do_id'] = $postData['submission_id'];
			$schoolNotificationData['school_id'] = $postData['school_id'];
			$schoolNotificationData['message'] = $postData['title']." assignment has been graded.";
			...
			//$this->sendMail($schoolEmail, "New Graded Assignment", $message);
			sendKidyviewEmail($schoolEmail,'','','','New Graded Assignment', $message);
			}
			$return['success'] = "true";
