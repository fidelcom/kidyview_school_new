...
class Reportparent extends REST_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('reports/Parent_model', 'parentreport');
    }
    ...
    public function getAllSchool_post() {
        ...
        $result = $this->parentreport->getAllSchool($countryID);
    ...
    public function getAllSectionClass_post() {
       ...
       $postData = json_decode(file_get_contents('php://input'), true);
        ...
        $result = $this->parentreport->getAllSectionClass($schoolid);
    ...
    public function getCountryList_post() {
        ...
        $result = $this->parentreport->getCountryList();
    ...
    public function getReport_post() {
        ...
        if (isset($postData['classSectionList']) && isset($postData['schoolLists']) && isset($postData['fromdate']) && isset($postData['todate'])) {
            ...
            $schoolParent = $this->parentreport->getParentReport($searchingArray);
            if ($schoolParent) {
                $return['success'] = "true";
                $return['message'] = "School Parent list";
                $return['data'] = $schoolParent;
                $return['error'] = $this->error;
                $this->response($return, REST_Controller::HTTP_OK);
            } else {
                $return['success'] = "false";
                $return['message'] = "No Parent found.";
                $return['error'] = $this->error;
                $return['data'] = '';
                $this->response($return, REST_Controller::HTTP_BAD_REQUEST);
            }
