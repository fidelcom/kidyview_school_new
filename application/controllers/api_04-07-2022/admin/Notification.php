<?php
	...
	class Notification extends REST_Controller {
		...
		public function __construct() {
			...
			$this->load->model('admin/Notification_model','model');
		...
		$limit=3;
		$countUnReadData = $this->model->getNotification('count',$teacherID,$adminID,'','');
		$countReadData = $this->model->getNotification('count',$teacherID,$adminID,'','All');
		$result = $this->model->getNotification('data',$teacherID,$adminID,$limit,'');
	...
	public function getAllNotification_post() {
		...
		$adminID=$adminData->id;
		$limit=100;
		$result = $this->model->getNotification('data',$teacherID,$adminID,$limit,'All');
		if ($result) {
			$return['success'] = "true";
			$return['message'] = "Notification found.";
			$return['data'] = $result;
	...
	public function deleteNotification_post() {
		...
		$adminID=$adminData->id;
		$adminID = "A-".$adminID;
		$where=array('receiver_id'=>$adminID);
		$data['is_delete']=1;
		$result = $this->model->updateNotification($where,$data);
		if ($result) {
			$return['success'] = "true";
