<?php
	...
	class User extends REST_Controller {
		...
		public function addParent_post() {
			...
			$result = $this->User_model->parentExist($fatheremail, $fatherphone);
			...
			if ($add) {
				$emailsubject = "<h2 style='margin:0; padding:5px 0 10px;margin-bottom:15px; border-bottom: 1px solid #bbb8af;color: #020a65;'>Your children's school Register you as Parent in Kidyview system.</h2>";
				...
				$mailResponse = $this->sendMailForCC($to, $cc1, $cc2, "A registration email for kidyview", $message);
			}
		...
		public function editParent_post() {
			...
			$result = $this->User_model->parentExistInEditCase($fatheremail, $fatherphone, $parentID);
			...
			if ($update) {
				$emailsubject = "<h2 style='margin:0; padding:5px 0 10px;margin-bottom:15px; border-bottom: 1px solid #bbb8af;color: #020a65;'>Your Account Edited in Kidyview system.</h2>";
		...
		public function getAllSubjectForSchool_post() {
			...
			$id = $postData['id'];
			$result = $this->User_model->getAllSubjectForSchool($id);
		...
		private function Send_Mail($to, $subject, $body) {
			...
			$from = "info@chawtechsolutions.com";
			$mail = new PHPMailer();
	...
	public function addSubadmin_post() {
		...
		$name = $postData['name'];
		$email = $postData['email'];
		$result = $this->User_model->subadminExist($email, $name);
	...
	public function editSubadmin_post() {
		...
		$result = $this->User_model->subadminExistInEditCase($email, $phone, $subadminID);
		if ($result) {
			$return['success'] = "false";
			$return['message'] = "Already Exists.";
			$return['error'] = $this->error;
		...
		$update = $this->User_model->editSubadmin($updateArr, $subadminId, $schoolId);
	...
	public function subadminDisabled_post() {
		...
		$update = $this->User_model->commonDisabled($updateArr, $id,'school_subadmin');
		if ($update) {
			$return['success'] = "true";
