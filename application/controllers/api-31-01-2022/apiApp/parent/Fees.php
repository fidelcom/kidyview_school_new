...
class Fees extends REST_Controller {
    ...
    public function feesinvoice_get($tx_ref){
       ...
       $this->db->group_by('sf.tx_ref');
        ...
        if ($query->num_rows() == 1) {
            $rowData= $query->row();
            $feesCategories = $this->model->feesMyCategories($rowData->cat_id);
            $rowData->categories = $feesCategories;
    ...
    public function invoicelist_get(){
       ...
       $this->db->join('fee_types as ft', ' ft.id = sf.feeType_id','left');
        ...
        $return['message'] = "Invoice List Found";
        $return['error'] = $this->error;
        $this->response($return, REST_Controller::HTTP_OK);    
        }
        else {
