...
public function addStudentHobie_post() {
	...
	public function getStudentsList_post() {
		...
		$result = $this->Student_model->getStudentsList($school_id,$postData);
			if ($result) {
					$return['success'] = "true";
	...
	public function childDisabled_post() {
			$postData = json_decode(file_get_contents('php://input'), true);
			if ($postData == '') {
				$postData = $_POST;
			}
            ...
            'status' => $status,
			 ...
			 $update = $this->Student_model->childDisabled($updateArr, $id);
			
			if ($update) {
				$return['success'] = "true";
				$return['message'] = "Student Status Disabled Now.";
				$return['error'] = $this->error;
