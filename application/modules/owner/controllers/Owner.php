...
class Owner extends MX_Controller {
    ...
    public function checkAllPrivilege($privelegeData) {
        ...
        if ($uri == 'editschool') {
            if ($privelegeData['SchoolManagement']['edit'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'subadminlist') {
        ...
        if ($uri == 'subscriptionlist') {
            if ($privelegeData['SubscriptionManagement']['view'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'addsubscription') {
            if ($privelegeData['SubscriptionManagement']['add'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'editsubscription') {
            if ($privelegeData['SubscriptionManagement']['edit'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'voucherlist') {
            if ($privelegeData['VoucherManagement']['view'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'addvoucher') {
            if ($privelegeData['VoucherManagement']['add'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'editvoucher') {
            if ($privelegeData['VoucherManagement']['edit'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'feesCategory') {
            if ($privelegeData['FeeCategory']['view'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'addfeescategory') {
            if ($privelegeData['FeeCategory']['add'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'editfeescategory') {
            if ($privelegeData['FeeCategory']['edit'] == 0) {
                $retrn = 0;
            }
        }
        if ($uri == 'loginimage') {
            if ($privelegeData['LoginImage']['view'] == 0) {
                $retrn = 0;
            }
        }
        if ($retrn == 0) {
            echo "<script>
            alert('You have not access permission for this.');
            window.location.href='" . base_url() . "owner';</script>";
            die;
        }
    }
    ...
    public function teacherView() {
        $this->load->view('teacherdetails');
    }
    public function teacherList() {
        $this->load->view('teacherListing');
    }
    ...
    public function subscriptionList() {
        $this->load->view('subscription/subscription-list');
    }
    public function addSubscription() {
        $this->load->view('subscription/add-subscription');
    }
    public function subscriptionView() {
        $this->load->view('subscription/subscription-detail');
    }
    public function editSubscription() {
        $this->load->view('subscription/edit-subscription');
    }
    public function loginimage() {
        $this->load->view('loginimage');
    }
    ...
    {
        $this->load->view('add-voucher');
    }
    public function editVoucher()
    {
        $this->load->view('edit-voucher');
    }
    ...
    public function reportParent() {
        $this->load->view('parent-report');
    }
    public function reportStudent() {
        $this->load->view('student-report');
    }
    public function reportTeacher() {
        $this->load->view('teacher-report');
    }
    public function reportDriver() {
        $this->load->view('driver-report');
    }
     public function reportRevenue() {
        $this->load->view('revenue-report');
    }
     public function reportSchool() {
        $this->load->view('school-report');
    }
    
    public function revenueGraph() {
        $this->load->view('revenue-graph');
    }
    
    public function sponser() {
        $this->load->view('sponser');
    }
    public function editsponser() {
        $this->load->view('editsponser');
    }
    ...
    public function mapcurrency() {
        $this->load->view('mapcurrency');
    }
    public function transactionLlist() {
        $this->load->view('transactionList');
    }
