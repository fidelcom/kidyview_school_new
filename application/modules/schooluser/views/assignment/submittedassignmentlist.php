...
<div class="app-main">
	...
	<!-- 	<div class="row rowFilter-head">
		<div class="col-md-4">
			<select class="form-control" ng-model="class_id" ng-change="getStudentSubmitedAssignmentList()"> 
				<option value="">Select Class</option>
				<option value="{{class.id}}" ng-repeat="class in classArray | unique : 'id'">{{class.name}}</option>
			</select>
		</div>
		<div class="col-md-4">
			<select class="form-control" ng-model="subject_id" ng-change="getStudentSubmitedAssignmentList()"> 
				<option value="">Select Subject</option>
				<option value="{{subject.id}}" ng-repeat="subject in subjectArray | unique : 'id'">{{subject.name}}</option>
			</select>
		</div>
		<div class="col-md-4">
			<select class="form-control" ng-model="category" ng-change="getStudentSubmitedAssignmentList()"> 
				...
				<thead>
					...
					<tbody>
						<tr ng-repeat="assignment in studentSubmitAssignmentData">
							...
							<td>
							<span ng-class="{'text-green':(assignment.assignment_status=='Graded' || assignment.assignment_status=='Marks Assigned')}">	{{assignment.assignment_status}}</span>
							</td>
							<td class="action text-right">
								<a href="#!/submited-assignment-detail/{{assignment.assignmentID}}"><i class="icon-eye" title="View"></i></a>
							</td>
						</tr>															
					</tbody>
				</table>
