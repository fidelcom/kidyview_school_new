						...
						<div class="question-block-e" ng-repeat="ques in questionList track by $index">
							...
							<div class="row">
								...
								<div class="col-md-12 mt-2 ans" ng-repeat="ans in ques.answer_value" ng-if="(ans.isUserAnswer=='true') || (ans.isUserAnswer==true) || (ans.isUserAnswer!='false') ">
									...
									<p  ng-if="(ques.question_type != 'textual') && (ans.option == ques.answer) &&  (studentExamDetails.marks_obtained !=null)" class="text-success"><i class="fa fa-check" aria-hidden="true"></i></p>

									<p ng-if="(ques.question_type != 'textual') && (ans.option != ques.answer) &&  (studentExamDetails.marks_obtained !=null)" class="text-danger"><i class="fa fa-close" aria-hidden="true"></i></p>
									...
									<div class="give-marks pull-right">
										<input type="text" class="form-control digit-only" ng-if="((ques.question_type == 'multiple') || (ques.question_type == 'boolean') || (ques.question_type == 'radio')) && (ans.option == ques.answer) && (studentExamDetails.exam_type == 'non-multiple')"  ng-model="ques.question_marks" ng-disabled="true" />
										
										<input type="text" class="form-control" ng-if="((ques.question_type == 'multiple') || (ques.question_type == 'boolean') || (ques.question_type == 'radio')) && (ans.option != ques.answer) && (studentExamDetails.exam_type == 'non-multiple')"  value="0" ng-disabled="true" />
									</div> 
								</div>

								<!-- Before Submitted Exam Enabled mark for textaul -->
								<input type="number" class="form-control digit-only" ng-if="(ques.question_type == 'textual') && (studentExamDetails.exam_type != 'multiple') &&  (studentExamDetails.marks_obtained == null)"  ng-model="questionList[$index].marks_obtain" placeholder="marks obtained" min="{{questionList[$index].marks_obtain}}" max="{{ ques.question_marks }}" />
								<!-- After Submitted Exam disabled mark for textaul -->
								<!-- <input type="number" class="form-control digit-only" ng-hide="(ques.question_type == 'textual') && (studentExamDetails.exam_type != 'multiple') &&  (studentExamDetails.marks_obtained !=null)"  ng-model="questionList[$index].marks_obtain" placeholder="marks obtained" min="{{questionList[$index].marks_obtain}}" max="{{ ques.question_marks }}" ng-disabled='true' /> -->
