	...
	$scope.addFeeSubscription = function(subscription){
		...
		$http.post(BASE_URL+'api/school/SubscriptionAmount/addFeesSubscription',subscription,{
				...
				var Gritter = function () {
					...
					$window.location.href = '#!/fee-suscription';
					// return false;
				}();
	...
	$scope.getAllStudentsAttendance = function()
	{
		...
		$http.post(BASE_URL+'api/school/attendance/getAllStudentsAttendance',formData,{
			...
			// return false
				...
				$scope.dates    	  = response.data.data.selected_monthDates;	
				$scope.colspan  	  = $scope.dates.length +2;
	...
	$scope.getAllParentsForSchool = function()
	{
		...
		$http.post(BASE_URL+'api/user/getAllParent',formData,{
			...
			if(response.status == 200)
			{
				...
				$window.location = BASE_URL+'tutor/login';
				}
				if(error.status==403){	
				console.log(error);
				return false;
...
function getAllTermCtrl($scope, $http, $routeParams, $timeout) 
{
	$scope.encryptStr = function(id)
	{
		var qry = id.toString();
...
function getAllStudentReportCtrl($scope, $http, $routeParams, $timeout) 
{
	$scope.getAllClassForSchool = function()
	{
		$scope.classList = false;
...
function allReportCtrl($scope, $http, $routeParams, $timeout) 
{
...
function schoolFAQCtrl($scope, $http, $window) 
{
	$scope.encryptStr = function(id)
	{
		var qry = id.toString();
...
function examCtrl($scope,$http,$window){
	// Get Class List
	$scope.getAllClassForSchool = function()
	{
		$scope.classList = [];
...
function schoolProjectCtrl($scope, $http, $route, $window) 
{
	$scope.encryptStr = function(id)
	{
		var qry = id.toString();
...
function schoolAssignmentCtrl($scope, $http, $route, $window) 
{
	$scope.encryptStr = function(id)
	{
		var qry = id.toString();
