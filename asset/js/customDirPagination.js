(function() {
    ...
    angular
        .module(moduleName)
        .directive('customDirPaginationControls', [
            'paginationService',
            'paginationTemplate',
            customDirPaginationControlsDirective,
        ]);

    function customDirPaginationControlsDirective(
        paginationService,
        paginationTemplate
    ) {
        var numberRegex = /^\d+$/;
        ...
        function dirPaginationControlsLinkFn(scope, element, attrs) {
            ...
            scope.autoHide =
                scope.autoHide === undefined ? true : scope.autoHide;
            ...
            scope.$watch(
                function() {
                    if (paginationService.isRegistered(paginationId)) {
                        return (
                            (paginationService.getCollectionLength(
                                paginationId
                            ) +
                                1) *
            ...
            function updateRangeValues() {
                if (paginationService.isRegistered(paginationId)) {
                    var currentPage = paginationService.getCurrentPage(
                        ...
                        itemsPerPage = paginationService.getItemsPerPage(
                            paginationId
                        ),
                        totalItems = paginationService.getCollectionLength(
                            paginationId
                        );
            ...
            function isValidPageNumber(num) {
                return (
                    ...
                    (0 < num && num <= scope.pagination.last)
                );
            }
        }
